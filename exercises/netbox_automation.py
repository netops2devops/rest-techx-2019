...
def push2netbox(mapping):
    ...
    headers = { "Authorization": "Token 2cf8f987c1361e672a8ec947b5bd40a9b5716e40", "Content-Type": "application/json" }
    
    for asnum, asname in mapping.items():
        info = { "name" : asname, "asn" : asnum, "slug" : randint(1,100000) , "description" : "peering point" }
        data = json.dumps(info)
        
        # Create new site with extracted ASN and ASNAME
        response = requests.post("http://0.0.0.0:32768/api/dcim/sites/", data=data, headers=headers, verify=False).json()
        pprint(response)
        print("\n")
